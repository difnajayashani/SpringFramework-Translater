...
public class PagemoveServlet extends HttpServlet {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        int pageNo=Integer.parseInt(initPage);
        LOG.info("casted page no : {}",pageNo);

        try {


            connection = dbPool.getConnection();
            LOG.info("Database connection obtained for user list based on pagination");

            st = connection.createStatement();
         /*   String sql ="SELECT * , DATE_FORMAT(birth_date,'%m/%d/%Y') AS niceDate FROM user LIMIT 10  WHERE username" +
                    " LIKE \"" +userSearched + "%\" ;";*/

            String sql=" SELECT *, DATE_FORMAT(birth_date,'%m/%d/%Y') AS niceDate FROM user LIMIT  5 OFFSET "+((pageNo-1)* 5 )+ ";";

            // String sql ="SELECT * , DATE_FORMAT(birth_date,'%m/%d/%Y') AS niceDate FROM user_data WHERE user_name REGEXP '^[\"" +userSearched + "\"].*$' ;";
            rs1 = st.executeQuery(sql);
            LOG.trace("Query to load user set based on pagination is executed ");
        ...
        LOG.info("User list output given: {} ", userList);
        out.print(userList);
    }
}
